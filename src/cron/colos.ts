...
// 						},
// 						body: "SELECT blob1 as worker, blob2 as Durable, count() FROM NewDOLatency GROUP BY blob1, blob2",
// 					},
// 				)).json<SQLResponse>()
// 			).data.reduce((a, b) => {
// 				if (!a[b.a]) {
	...
	return Object.fromEntries(
		Object.entries(
			(
				...
				a[b.worker][b.durable] = a[b.worker][b.durable] ? a[b.worker][b.durable] + 1 : 1;
				return a;
			}, {} as SQLParsed),
		)
			.map((e) => {
